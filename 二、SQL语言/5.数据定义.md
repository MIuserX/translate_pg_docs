...
5. 我们想要应用程序执行 `INSERT INTO measurement ...` 命令时数据被影射到合适的子表中。我们可以通过向 master 表添加一个触发器函数来完成这个目标。如果数据仅仅被添加到最近的子表中，用一个简单的触发器函数就能做到：
   ...
   `COPY` 是忽略规则的。如果你想用 `COPY` 来插入数据，你需要插入正确的子表，而不是插入到 master 表中。`COPY` 不会启动触发器，所以当使用触发器方案时可以正常使用。
   
   规则方案的另一个缺点是，没有简单的方法在规则没有覆盖插入数据的日期时产生一个报错，数据会被默默地保存到 master 表中。
   
6. 确保 [constraint_exclusion](https://www.postgresql.org/docs/13/runtime-config-query.html#GUC-CONSTRAINT-EXCLUSION) 配置参数在 postgresql.conf 中没有被禁用，否则子表会被不必要的访问。



​		如我们所见，一个复杂的表层次可能需要大量的 DDL。在上述例子中，我们可以每月创建一个子表，所以写一个脚本自动每月生成 DDL 是非常方便的。
...
​		仅在继承层次中删除子表但保留对子表的访问：

```sql
ALTER TABLE measurement_y2006m02 NO INHERIT measurement;
```

​		要添加一个新子表处理新数据，创建一个空的像上面那样原始子表的子表：
...
​		作为替代的方法，你可能想创建子表，并在后续再将它加入继承层次。这允许数据被加载、检查、与传送，在被父表中可见前。
...
* 没有自动化的方法来确认所有的 check 约束是互相唯一的。更安全的方法是写代码来生成子表、创建后修改相关的对象，而不是手写。

* 索引和外键约束适用个单个表而不适用于它们的继承子代，所以他们有一些需要注意的 [说明](https://www.postgresql.org/docs/13/ddl-inherit.html#DDL-INHERIT-CAVEATS)。

* 展示在这里的 schema 假定行的 key 列的值是不变的，或者说不会改变到需要它移动到其他分区的程度。一个尝试这样做的 `UPDATE` 会失败因为 check 约束。如果你要处理这样的情况，你可以在子表上设置合适的更新触发器，但是这使得结构的管理更加复杂。

* 如果你正在使用人工 `VACUUM` 或 `ANALYZE` 命令，不要忘记你需要在每个子表上运行它们。一个命令类似：
...
* 带 `ON CONFLICT` 的 `INSERT` 语句，不可能按照预期那样去工作，因为 `ON CONFLICT` 被视为在指定表上检查是否违反唯一约束，不是它的子表。

* 需要触发器或规则来将新行映射到合适的子表中去，除非应用程序能显式地感知到分区方式。触发器可能会很复杂，并且会比声明式分区运行的更慢。



#### 5.11.4 分区修剪

​		***分区修剪*** 是一种查询优化技术，用来改进声明式分区表的性能。一个例子：
...
​		如果没有分区修剪，上面的查询会扫描 `measurement` 表的每个分区。如果允许分区修剪，规划器会检查每个分区的定义并证明这个分区不需要被扫描，因为这个分区可能不包含任何满足查询的 where 条件的行。当规划器可以证明这个的时候，它会从查询计划中修剪掉这个分区。

​		使用 `EXPLAIN` 命令和 [enable_partition_pruning](https://www.postgresql.org/docs/13/runtime-config-query.html#GUC-ENABLE-PARTITION-PRUNING) 配置参数，展示修剪了分区和没修剪分区的差异是可能的。一个对这种类型的表设置的典型的未优化的查询计划：
...
​		一些或全部分区将使用索引扫描代替全表顺序扫描，但关键点时要得出这个查询的结果并不需要扫描老分区。当允许分区修剪时，我们会拥有一个非常简单的查询计划：
...
​		注意，分区修剪仅仅被分区 key 隐式定义的约束驱动，而不是被涉及到的索引驱动。因此，不必在分区 key 的列上定义索引。某个分区是否需要创建索引，取决于你是否想要扫描分区的查询扫描分区的大部分还是小部分。对于后面的场景索引是有帮助的，但对于前面的并不需要。

​		分区修剪不仅仅在为查询设计查询计划的时候被执行，也在查询的执行时被执行。这个策略是有益的，因为这可以使得更多的分区被修剪掉，当查询的子句中包含设计查询计划阶段无法得知其值的表达式时。例如，参数定义在 `PREPARE` 语句中，使用了一个从子查询中获取的值 或者 在 JOIN 的嵌套循环的内层循环使用了参数化的值。查询执行阶段的分区修剪可以在以下任何一个时间点执行：

* 在查询计划初始化时。对于在执行的初始化阶段，值就已知的参数，可以执行分区修剪。在这个阶段被修剪的分区不会出现在查询的 `EXPLAIN` 和 `EXPLAIN ANALYZE`。在这个阶段获知被修剪掉的分区的数量时可能的，可以通过观察 `EXPLAIN` 数据的 "Subplans Removed" 。
* 在查询计划实际执行时。使用只有在执行阶段才能得知的值，来进行分区修剪。这些值包括来自子查询的 或者 来自执行时参数(例如来自 JOIN 的嵌套循环的参数化的值)。因为这些参数的值可能在查询执行的时候改变很多次，无论何时能影响修剪分区的参数被使用，都进行分区修剪操作。要获知分区是否在这个阶段被修剪需要仔细地观察 `EXPLAIN ANALYZE`  输出的 `loops` 属性。不同分区的子计划可能拥有不同的值，取决于它们在执行时被修剪的次数。一些可能被展示为 `(never executed)` ，如果它们每次都被修剪的话。



​		可以使用 [enable_partition_pruning](https://www.postgresql.org/docs/13/runtime-config-query.html#GUC-ENABLE-PARTITION-PRUNING) 参数来禁止分区修剪。

> **Note**
>
> 执行时分区修剪当前只会出现在 Append 和 MergeAppend 节点类型。对于 ModifyTable 节点类型这还没有实现，但是这个情况可能在未来发布的版本里有所改变。



#### 5.11.5 分区与约束排除

​		***约束排除*** 是一个与分区修剪相似的查询优化技术。这主要用于继承方法实现的分区，这也可以被用于其他目的，包括与声明式分区一起用。

​		约束排除的工作原理类似于分区修剪，除了它使用每个表的 check 约束(这赋予了这种方法的名字)，而分区修剪使用了分区的边界(边界仅存在于声明式分区)。另一个不同是：约束排除仅应用于计划阶段，不会尝试在执行阶段排除分区。

​		使用 check 约束使得约束排除比分区修剪慢，但这有时也可作为一个优点：因为约束甚至可以定义在声明式分区表上，作为它们内部的分区边界的补充，约束排除可以排除来自查询计划的额外的分区。

​		[constraint_exclusion](https://www.postgresql.org/docs/13/runtime-config-query.html#GUC-CONSTRAINT-EXCLUSION) 的默认(也是推荐的)配置不是 `on` 也不是 `off`，而是叫做 分区 的中间设置，这个配置使得这个技术仅仅被应用于可能使用继承方案的分区表。设置为 `on`，规划器会在所有查询中检查 check 约束，甚至那些非常简单的，不太可能得到收益的查询也会使用。

​		下面的说明应用于约束排除：

* 约束排除 仅被应用在规划查询阶段，不像分区修剪那样可以被应用到查询执行阶段。
* 约束排除 仅在查询的 where 条件中包含约束(或外部提供参数) 时工作。例如，一个非不变函数参与的比较运算，像 `CURRENT_TIMESTAMP` 函数不能被优化，因为规划器不知道运行时这个函数的值会导致查询进入哪个子表。
* 保持分区约束的简单，否则规划器将可能无法证明子表不需要被访问。对于 list 分区，使用简单的等于表达式，对于 range 分区，使用简单的范围测试，如前面的例子阐述的那样。一个好的规则是：分区约束应该只包含，使用可建立 B-Tree 索引的比较分区列和常量的操作符，因为只有可建立 B-Tree 索引的列才可以作为分区 key。
* 父表的所有子表的约束会在约束排除时被检测，子表数量较大可能会大大增加查询的规划时间。所以基于继承的分区表，100个子表以内，会工作的较好，不要尝试使用数千个子表。



#### 5.11.6 声明式分区的最佳实践

​		如何将一个表进行分区的决定需要认真谨慎的思考，不良的设计会给查询的规划和执行的性能带来负面影响。

​		最关键的设计决策之一是：你的数据通过哪些列来进行分区。通常最佳选择是将分区表上查询的 where 子句中最常用的哪些列作为分区 key。where 子句...可以被用来修剪不需要的分区。但是，主键和唯一约束的要求可能会强制你作出一些决定。当规划你的分区策略时，删除不需要的数据也是一个要考虑的因素。一个完整的分区可以被公平快速的删除，所以，以这种方式来设计是有益的：需要一次性删除的数据都位于一个分区。

​		另一个关键决策是：表要被分为几个分区。没有足够的分区可能意味着 索引会很大 或者 数据位置很少(这会导致缓存命中率较低)。但是，将表分为过多的分区也会造成问题。过多的分区会造成查询规划消耗更长的时间、查询规划和执行时会有更高的内存消耗(下面深入描述)。当选择如何对你的表进行分区时，考虑未来会发生什么变化也是非常重要的。例如，如果你选择为每个客户都创建一个分区，且当前你拥有少量的大客户，考虑过几年你发现你拥有大量的小客户有可能才是事实。在这个案例中，使用 HASH 分区且选择合理的分区数量可能是更好的选择，而不是尝试  LIST 分区并期望用户数量不会增长超高实际的分区数量。

​		对那些被期望变成比其他分区更大的分区使用子分区来进一步分区可能是更有用的。另一个选择是使用 range 分区，在分区 key 中使用多个列。任何一个方案都极易导致大量分区，所以建议考虑控制。

​		考虑在查询的规划和执行期间分区的开销是重要的。一般，查询规划器把至多几千个分区的分区结构处理的非常好，提供了 典型的查询允许查询规划器修剪所有(但量不大)分区。规划器执行了分区修剪之后还剩下很多分区时，查询规划的时间会变的更长、内存消耗会变的更大。对于 `UPDATE` 和 `DELETE` 命令这是非常正确的。关于拥有大量分区，另一个需要关心的问题是，服务器的内存消耗可能会随着时间显著增长，尤其是当很多会话创建大量分区时。那是因为每个分区需要把元数据加载到会话的当前内存。

​		对于数据仓库式的工作量，使用大量分区是比 OLTP 类型的工作量更有意义。一般，在数据仓库中，查询规划时间在查询执行期间是很少关注的。对于这两种工作负载的任何一个，早期作出正确的决策是重要的，因为重分区是非常慢的。工作负载的仿真对于优化分区策略经常是有好处的。不要轻率地假设更多的分区比更少的分区好，反之亦然。



### 5.12 外部数据

​		pg 实现了 SQL/MED 规格的一部分，允许你使用一般的 SQL 访问 pg 以外的数据。这类数据被称为 ***外部数据(foreign data)***。(注意，这个用法不要跟外键混淆，外键是数据库内部的一种约束类型)

​		访问外部数据需要 ***foreign data wrapper***。一个 foreign data wrapper 是一个可以与外部数据源通信的库，隐藏了连接数据源和从数据源获取数据的细节。contrib 模块提供了一些可用的 foreign data wrapper ，详情查看 [Appendix F](https://www.postgresql.org/docs/13/contrib.html) 。其他类型的 foreign data wrapper 要以第三方产品出现。如果已有的 foreign data wrapper 不能满足你的要求，你可以写一个，看 [Chapter 56](https://www.postgresql.org/docs/13/fdwhandler.html) 。

​		要访问外部数据，你需要创建一个数据库对象：***foreign server***，foreign server 定义了根据配置如何连接一个特定的外部数据源。然后你需要创建一或多个 ***foreign tables***，foreign table 定义了远程数据的结构。foreign table 可以像正常的表那样被用在查询中，但 foreign table 的数据实际上没保存在 pg server 上。无论何时使用，pg 向 foreign data wrapper 获取来自外部数据源的数据，或者传输数据到外部数据源在 `UPDATE` 命令被使用时。

​		访问远程数据可能需要向外部数据源认证身份。认证信息的提供可以使用 ***user mapping***，user mapping 可以提供额外的数据，例如用户名、密码，基于当前 pg 用户的。

​		要获取更多信息，看 [CREATE FOREIGN DATA WRAPPER](https://www.postgresql.org/docs/13/sql-createforeigndatawrapper.html) 、[CREATE SERVER](https://www.postgresql.org/docs/13/sql-createserver.html) 、[CREATE USER MAPPING](https://www.postgresql.org/docs/13/sql-createusermapping.html) 、[CREATE FOREIGN TABLE](https://www.postgresql.org/docs/13/sql-createforeigntable.html) 、[IMPORT FOREIGN SCHEMA](https://www.postgresql.org/docs/13/sql-importforeignschema.html) 。



### 5.13 其他数据对象

​		表是关系型数据库结构的核心对象，因为表存储了你的数据。但数据库中并不是只有 表 这种对象类型。很多其他类型的对象也可以被创建，用来使用、管理数据，更加高效或方便。本章节并没有讨论这些对象类型，但这里我们给出了一个列表，让你意识到你可以干什么：
...
​		这些主题的细节请看 [Part V](https://www.postgresql.org/docs/13/server-programming.html) 。



### 5.14 依赖跟踪

​		当你创建包含很多带外键约束的表、视图、触发器、函数等的复杂数据库结构，你隐式地创建了一个在这些对象之间的依赖网络。举例来说，一个带外键约束的表依赖于它引用的表。

​		要保证整个数据库结构的完整性，pg 不会让你删除还被其他对象依赖的对象。例如，尝试删除我们在 [Section 5.4.5](https://www.postgresql.org/docs/13/ddl-constraints.html#DDL-CONSTRAINTS-FK) 中提到的产品表，产品表被订单表依赖，这会导致像下面这样的报错信息：
...
```
 		...
 		报错信息包含有用的提示：如果你嫌一个个地删除所有依赖的对象非常麻烦，你可以使用：

```sql
DROP TABLE products CASCADE;
```

​		这时所有的依赖对象会被删除，而且依赖于这些被删除对象的对象也会被递归删除。在这个例子中，这不会删除订单表，仅仅会删除外键约束。删除会在删除外键约束后停止，因为没有其他对象依赖于外键约束。(如果你想检查 `DROP ... CASCADE` 会做什么，运行 `DROP` 命令不要加 `CASCADE` 然后看输出的细节。)

​		pg 中几乎所有的 `DROP` 命令支持指定 `CASCADE`。当然，依赖于待删除对象的对象种类可能因待删除对象的类型而不同。你可以可以写 `RESTRICT` 来代替 `CASCADE` 来执行默认的行为，默认行为是保护被其他对象依赖的对象。

> **Note**
>
> ​		根据 SQL 标准，指定 `RESTRICT` 或 `CASCADE` 在 `DROP` 命令中是必须的。实际上没有哪个数据库系统强制用户遵守这个规则，但默认行为是 `RESTRICT` 或 `CASCADE` 取决于具体的系统实现。

​		如果一个 `DROP` 命令中列出多个要删除的对象，只有在指出的列表之外的对象存在依赖，才需要写 `CASCADE`。例如，执行 `DROP TABLE tab1, tab2`，tab2 存在外键引用到 tab1，这并不需要写 `CASCADE`。

​		对于用户自定义的函数，pg 会跟踪函数的外部可见的特性的依赖，例如参数类型和结果类型，但不会跟踪哪些在函数体内部才能得知的东西。一个例子：
...
​		(看 [Section 37.5](https://www.postgresql.org/docs/13/xfunc-sql.html) 获取关于 SQL 函数的知识。) pg 只会意识到 `get_color_note` 函数依赖于 `rainbow` 类型：删除这个类型会导致这个函数也被删除，因为它的参数类型已变成未定义。但 pg 不认为 `get_color_note` 依赖 `my_colors` 表，所以这个表被删除时，函数并不会被删除。这种策略有一些缺点，但也有优点。如果这个表没了，这个函数在一些意义上仍是合法的，虽然执行它会导致报错，但只要创建新表拥有一样的名字的，这个函数就可以再次运行。