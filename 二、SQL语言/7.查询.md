...
​		[GROUP BY](https://www.postgresql.org/docs/13/sql-select.html#SQL-GROUPBY) 子句被用来将被列出的列上值相等的行分成一组。被列出的列的顺序不重要。效果是将所有拥有相同值的行合并为一行来表示组中的所有行。这么做是为了删除冗余或者在分组上应用聚合函数。例如：
...
​		一般，如果一个表被分组，`GROUP BY` 中没列出的列，不可以被引用，除非在聚合表达式中。一个使用聚合表达式的例子是：
...
​		在这里 `sum` 是个聚合函数，它基于整个组计算出一个单个值。更多关于聚合函数的信息可以看 [Section 9.21](https://www.postgresql.org/docs/13/functions-aggregate.html)。
...
​		下面是另一个例子：它计算每个产品的总销额(而不是所有产品的总销额)：
