...
​		***aggregate_name*** 是一个事先定义的聚合(可能带着一个 schema 前缀)，***expression*** 是任何值表达式且自身不包含一个聚合表达式或一个窗口函数调用。可选的 ***order_by_clause*** 和 ***filter_clause*** 在后面描述。
...
​		如果 `DISTINCT` 在 order_by_clause 中被使用了，
