...
​		***aggregate_name*** 是一个事先定义的聚合函数(可能带着一个 schema 前缀)，***expression*** 可以是任何值表达式且自身不包含一个聚合表达式或一个窗口函数调用。可选的 ***order_by_clause*** 和 ***filter_clause*** 在后面描述。

​		第一种形式的聚合函数调用表达式调用了聚合函数一次，对于所有的输入行。第二种形式与第一种一样，因为 `ALL` 是默认的。第三种形式对于每个唯一的值调用一次聚合函数(或者对于每个唯一的值的结合，因为可以写多个表达式)。第四种形式对于输入行调用一次聚合函数；因为没有特定的值被指定，这通常只对 `count(*)` 有用。最后一种形式与 ordered-set 聚合函数一起用，这在下面会讲到。

​		大部分聚合函数忽略 NULL，所以那些一个或多个表达式为 NULL 的行会被抛弃。这个规则对于所有内置聚合函数都是生效的，除非额外指出。

​		例如，`count(*)` 计算输入行的个数；`count(f1)` 计算 `f1` 不为 NULL 的行的个数，因为 `count` 会忽略 null 值；`count(distinct f1)` 计算非NULL的 `f1` 的值有几种。

​		一般，输入行被无序的输入给聚合函数。大多数情况下这个问题不重要；例如，`min` 总是输出相同的结果，无论输入行的顺序是什么。然而，有一些聚合函数(例如 `array_ag` 和 `string_agg`) 输出的结果依赖于输入行的顺序。当使用这种聚合函数时，可选的 ***order_by_clause*** 可以被用来指定顺序。order_by_clause 的语法与查询级别的 `ORDER BY`子句拥有相同的语法，这在 [Section 7.5](https://www.postgresql.org/docs/13/queries-order.html) 中被描述，但聚合函数的 order_by_clause 中的表达式只能是表达式，而不能是输出列的列名或数字。例如：

```sql
SELECT array_agg(a ORDER BY b DESC) FROM table;
```

​		当使用多参数的聚合函数时，注意 `ORDER BY` 子句在所有的参数之后。例如：
...
​		后一种形式在语法上是不正确的，但它表示的是一个有两个排序表达式的具有单参数的聚合函数调用()。
...
​		一个行构造器可以包含 `rowvalue.*` 语法，在这个语法中，`rowvalue.*` 会被扩展为 `rowvalue` 的所有域的值，
